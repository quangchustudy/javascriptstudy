...
module.exports.handler = async (event, context) => {
    const ecs_services = SERVICE_NAME.map((serviceName) => ({service: serviceName, cluster: CLUSTER_NAME,  desiredCount: 1}));
    ...
    try {
        ...
        console.log(`Failed to set desired task count to 1 for services: ${ecs_services.map(service => service.service)}`);
        console.log(err);
        return err;
    }
    return {
        statusCode: 200,
        body: JSON.stringify({
            message: `Successfully set desired task count to 1 for services: ${ecs_services.map(service => service.service)}`,})
    }
}
